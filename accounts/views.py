...
from hierarchy.models import Branch,Organisation
...
class LogInView(GuestOnlyView, FormView):
    ...
    def form_valid(self, form):
        ...
        logging.debug("user is logged in",form.user_cache)
...
class DashboardView():
    ...
    def dashboard_index(request):
        context = {
            'pagetitle': 'Radiant HRM | Dashboard',
        }

        #Set Emolyee data in session
        adminfor = False
        adminforId = False
        request.session['is_studentrole_needed'] = 1
        organisationdata=False
        if(request.user.editor_type>0):

            if(request.user.editor_type==1):
                adminfor='fr'
                branchData=Branch.objects.get(pk=request.user.editor_typeid)
                adminforId=branchData.organisation_id
                organisationdata = Organisation.objects.get(pk=branchData.organisation_id)
            elif(request.user.editor_type==2):
                adminfor='ce'
                adminforId = request.user.editor_typeid
                
                branchData = Branch.objects.get(pk=request.user.editor_typeid)
                organisationdata = Organisation.objects.get(pk=branchData.organisation_id)

            request.session['is_studentrole_needed'] = str(organisationdata.is_studentrole_needed)
        request.session['adminfor'] = adminfor
        request.session['adminforId']=adminforId
        request.session['emailid'] = request.user.email
